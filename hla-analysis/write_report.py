#!/usr/bin/env python3
"""Generate an intermediate report without claiming the five-part goal complete."""
import json
...
def main():
    seq=json.loads((OUT/'sequence_summary.json').read_text())
    candidates=json.loads((OUT/'coding_candidates_summary.json').read_text())
    within=json.loads((OUT/'within_type_summary.json').read_text())
    discord=json.loads((OUT/'experimental_discordances_summary.json').read_text())
    metrics=table(OUT/'typing_metrics.tsv')
    lookup={(r['gene'],r['method']):r for r in metrics if r['cohort']==r['rarity']=='ALL' and r['split']=='leave_family_out'}
    lines=['# Expanded HLA analysis — intermediate results','',
        ...
        '|---|---:|---:|---:|---:|']
    for gene,r in seq.items(): lines.append(f"| {gene} | {r['n']} | {r['complete']} | {r['exact_genomic']} | {r['exact_cds']} |")
    lines += ['',
        ...
        f"There are {candidates['candidate_proteins']} distinct database-absent protein signatures in {candidates['entries']} complete-CDS entries. All remain unresolved; none has yet been validated as a new allele. `coding_candidates.tsv` provides stable sequence-based IDs, nearest reference protein edit distances, source donors and quality flags.", '',
        ...
        '|---|---:|---:|---:|---:|---:|---:|']
    for gene in sorted(seq):
        vals=[]
        for method in ['gene_sequence','coding_sequence_all_31mers','gene_bundles_fixed_panel','classII_bundles_fixed_panel','gene_content']:
            r=lookup.get((gene,method)); vals.append(f"{float(r['accuracy_all']):.1%}" if r else '—')
        lines.append('| '+gene+' | '+lookup[gene,'gene_sequence']['n']+' | '+' | '.join(vals)+' |')
    lines += ['',
        ...
        f"Current frozen G-group definitions make {discord.get('compatible_with_antigen_exon_resolution',0)} of those disagreements compatible with antigen-recognition-exon resolution; {discord.get('unresolved_antigen_exon_discordance',0)} remain unresolved at that level. Expanding historical partial allele names gives possible compatibility, not proof of the original assay's exact sequence. `experimental_discordances.tsv` preserves the evidence; read/alternative-assembly inspection is still required. [G-group construction](https://hla.alleles.org/pages/p%26g_groups/g_groups/) can also infer unsequenced regions, further limiting this compatibility check.", '',
        '## Information within matching types','',
        f"Among {within['haplotypes']} retained haplotypes, {within['all_classical_label_eligible']} have usable current labels at all eight core loci and resolved annotated DRB3/4/5 copies. Unannotated DRB3/4/5 states remain provisional, not confirmed absence. The screen records {within['contrasts']['drdq_label']} feature contrasts within matching DRB1–DQA1–DQB1 labels and {within['contrasts']['all_classical_label']} within all available classical two-field labels. These are correlated contrasts, not counts of independent variants or discoveries.", '',
        ...
        '```sh','python3 hla-analysis/sequence_catalogue.py','python3 hla-analysis/coding_candidates.py','OPENBLAS_NUM_THREADS=2 OMP_NUM_THREADS=2 python3 hla-analysis/benchmark.py','python3 hla-analysis/within_type_catalogue.py','python3 hla-analysis/experimental_discordances.py','OPENBLAS_NUM_THREADS=2 python3 hla-analysis/benchmark_uncertainty.py','python3 hla-analysis/check_results.py','python3 hla-analysis/write_report.py','```','',
    ...
    lines += ['## Targeted validation and incremental bundle test', '',
    ...
    lines += ['## Structural catalogue and final panel eligibility checkpoint', '',
    ...
    (BASE/'INTERMEDIATE_REPORT.md').write_text('\n'.join(lines))
